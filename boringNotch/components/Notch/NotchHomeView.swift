...
//  Updated 2025-11-02 calendar logic when paused/stopped
//
...
struct AlbumArtView: View {
    ...
    private var appIconOverlay: some View {
        if vm.notchState == .open && !MusicManager.shared.usingAppIconForArtwork {
            AppIcon(for: MusicManager.shared.bundleIdentifier ?? "com.apple.Music")
                .resizable()
                .aspectRatio(contentMode: .fill)
...
struct NotchHomeView: View {
    ...
    // Track Plex playback/polling
    @ObservedObject private var plexVM = PlexNowPlayingViewModel.shared
    // Track system music playback (Apple Music / Spotify through MusicManager)
    ...
    /// True if any playback is active (Plex or system player).
    private var anyPlaybackActive: Bool {
        // `hasActivePlayback` is expected to exist in PlexNowPlayingViewModel (snapshot != nil && !paused).
        plexVM.hasActivePlayback || music.isPlaying
    }

    /// Show mini-player only when playback is active.
    private var miniPlayerVisible: Bool { anyPlaybackActive }

    /// Show calendar when enabled AND no playback is active.
    private var calendarVisible: Bool { Defaults[.showCalendar] && !anyPlaybackActive }

    private var showShuffleAndRepeat: Bool {
        !(shouldShowCamera && calendarVisible) && Defaults[.showShuffleAndRepeat]
    }

    private var mainContent: some View {
        HStack(alignment: .top, spacing: (shouldShowCamera && calendarVisible) ? 10 : 15) {
        ...
        .animation(.smooth, value: miniPlayerVisible)
        .animation(.smooth, value: calendarVisible)
        .transition(
...
// MARK: - Slider helpers (kept here to avoid “type not found”)
...
struct MusicSliderView: View {
    ...
    // Compute elapsed time based on current timeline date
...
struct CustomSlider: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            let progress = span == .zero ? 0 : (value - range.lowerBound) / span
            ...
            .highPriorityGesture(
                DragGesture(minimumDistance: 0)
                    .onChanged { g in
                        ...
                        let newValue = range.lowerBound + Double(g.location.x / width) * span
